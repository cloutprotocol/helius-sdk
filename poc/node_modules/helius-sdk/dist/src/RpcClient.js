...
class RpcClient {
    ...
    sendSmartTransaction(instructions_1, signers_1) {
        return __awaiter(this, arguments, void 0, function* (instructions, signers, lookupTables = [], sendOptions = {}) {
            try {
                const { transaction } = yield this.createSmartTransaction(instructions, signers, lookupTables, sendOptions);
    ...
    sendSmartTransactionWithWalletAdapter(instructions_1, payer_1, signTransaction_1) {
        return __awaiter(this, arguments, void 0, function* (instructions, payer, signTransaction, lookupTables = [], sendOptions = {}) {
            try {
                const { transaction } = yield this.createSmartTransactionWithWalletAdapter(instructions, payer, signTransaction, lookupTables, sendOptions);
                return this.broadcastTransaction(transaction, sendOptions);
            }
            catch (error) {
     ...
     * ```
     */
    getProgramAccountsV2(programId_1) {
        return __awaiter(this, arguments, void 0, function* (programId, options = {}) {
            try {
                const url = `${this.connection.rpcEndpoint}`;
                // Build params array
                ...
                if (Object.keys(options).length > 0) {
                    const configOptions = {};
                    if (options.encoding)
                        configOptions.encoding = options.encoding;
                    if (options.commitment)
                        configOptions.commitment = options.commitment;
                    ...
                    if (options.limit !== undefined)
                        configOptions.limit = options.limit;
                    ...
                    if (options.filters)
                        configOptions.filters = options.filters;
     ...
     * ```
     */
    getAllProgramAccounts(programId_1) {
        return __awaiter(this, arguments, void 0, function* (programId, options = {}) {
            const allAccounts = [];
            ...
            const limit = 10000;
            do {
                const response = yield this.getProgramAccountsV2(programId, Object.assign(Object.assign({}, options), { limit,
                    paginationKey }));
     ...
     * ```
     */
    getTokenAccountsByOwnerV2(ownerAddress_1, filter_1) {
        return __awaiter(this, arguments, void 0, function* (ownerAddress, filter, options = {}) {
            try {
                const url = `${this.connection.rpcEndpoint}`;
                // Build params array
                const params = [ownerAddress, filter];
                // Add options if provided
                if (Object.keys(options).length > 0) {
                    const configOptions = {};
                    if (options.encoding)
     ...
     * ```
     */
    getAllTokenAccountsByOwner(ownerAddress_1, filter_1) {
        return __awaiter(this, arguments, void 0, function* (ownerAddress, filter, options = {}) {
            const allAccounts = [];
            let paginationKey = undefined;
            // Always use maximum limit for auto-pagination to minimize API calls
            const limit = 10000;
            do {
                const response = yield this.getTokenAccountsByOwnerV2(ownerAddress, filter, Object.assign(Object.assign({}, options), { limit,
                    paginationKey }));
   ...
   * }, wallet);
    ...
    executeJupiterSwap(params, signer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const swapResponseRaw = yield fetch('https://lite-api.jup.ag/swap/v1/swap', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                const signature = yield this.broadcastTransaction(transaction, {
                    skipPreflight,
                    maxRetries,
    ...
    broadcastTransaction(transaction_1) {
        return __awaiter(this, arguments, void 0, function* (transaction, options = {}) {
            var _a;
            const { lastValidBlockHeightOffset = 150, pollTimeoutMs = 60000, pollIntervalMs = 2000, pollChunkMs = 10000, skipPreflight = false, preflightCommitment = 'confirmed', maxRetries = 0, } = options;
            if (lastValidBlockHeightOffset < 0) {
                throw new Error('lastValidBlockHeightOffset must be a positive integer');
            }
            let serializedTx;
